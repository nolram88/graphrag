...
class MilvusVectorStore(BaseVectorStore):
    ...
    def search_by_id(self, id: str) -> VectorStoreDocument:
        ...
        try:
            # Build expression based on ID type
            expr = f'{self._id_field} == "{id}"' if isinstance(id, str) else f'{self._id_field} == {id}'

            res = self.db_connection.query(
                collection_name=self.collection_name,
                filter=expr,
                output_fields=["*"]
            )
            ...
            doc = res[0]  # Get first match
            _id = doc.pop(self._id_field)
            text = doc.pop(self._text_field)
            vector = doc.pop(self._vector_field)

            return VectorStoreDocument(
                id=_id,
                text=text,
                vector=vector,
                attributes=doc  # Remaining fields become attributes
            )

        except Exception as e:
            logger.error(f"Error searching Milvus by ID: {str(e)}")
            return VectorStoreDocument(id=id, text=None, vector=None)
